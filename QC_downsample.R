...
savePlots <- FALSE
loadQCed <- TRUE  # if load already QC'ed data.
...
if (loadQCed == FALSE){
    ...
    timeStamp("QC'ed Data loading...")
    # count matrix
    counts <- readMM(file="counts_QCed_1000gene10MT.mtx.gz")
    # cell ID
    cells <- read_csv("cellNames_QCed_1000gene10MT.csv", HEADER=FALSE)
    # gene ID
    genes <- read_csv("geneNames_nonZeroRemoved.csv")
    # metadata
    anno <- read_csv("annotation_QCed_1000gene10MT.csv")

    rownames(counts) <- genes$x
    colnames(counts) <- cells$x

    timeStamp("QC'ed data loaded.")
}
